...
final_df = deduped_df.drop( col("lcFirstName"), col("lcLastName"), col("lcEmail"))  # Drop col(lcFirstName), col(lcLastName), col(lcEmail)
...
# DBTITLE 1,Challenge: Build Optimized Reporting Pipeline
...
franchise_report_df = (enriched_transactions_df
    .groupBy(
          "franchiseID",
          usa_franchises_df["name"].alias("franchise_name"),
          usa_franchises_df["city"].alias("franchise_city"),
          usa_franchises_df["country"].alias("franchise_country")
    )
    .agg(
...
# DBTITLE 1,Diagnose DataFrame Row Counts and Samples
# Diagnose DataFrame row counts and sample data
print("transactions_df count:", transactions_df.count())
print("clean_customers_df count:", clean_customers_df.count())
print("usa_franchises_df count:", usa_franchises_df.count())

print("\nSample transactions_df:")
display(transactions_df.limit(5))

print("\nSample clean_customers_df:")
display(clean_customers_df.limit(5))

print("\nSample usa_franchises_df:")
display(usa_franchises_df.limit(5))
